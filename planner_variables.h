...
vector<int> dest;
map<int,int > trans_cost;
map<int, map<int, vector< vector<int> > > > cost_filter;
map<int, map<int, vector< vector<int> > > > neg_trans_filter;
//map<int, map<int, vector <int > > > neg_trans_filter;
map<int, map<int, vector<int> > > fsytrans;
map<int, vector<int> > fsystate;
...
{
    int x,y,s,cf,f;
    point1(int a,int b,int s,int cf,float f)
    {
        x=a;
        y=b;
	s=s;
...
unordered_map<int,vector<int> > neg_trans_fil_to_neighbour;
...
trans_system_path_node* newtrans_system_path_node(int x,int y,int s,int cf)
{
    trans_system_path_node* nd = new trans_system_path_node;
    nd->coord.x=x;
    nd->coord.y=y;
...
prod_graph_node* new_prod_graph_node(int p,int s,int y,int x,int cf)
{
