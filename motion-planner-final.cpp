...
float cal_actual_distance_bw_points(point a,point b,int cf,int par,int t_cst);
...
float cal_actual_distance_bw_points(point srcnode,point dest_node,int cf,int par,int t_cst)
{
    ...
    start_node.g = 0;
    start_node.h = t_cst+cal_heuristic_cost(start_node.coord,dest_node);
...
int astar_path_src_to_dest(prod_graph_node* src,prod_graph_node* dest)
{
	...
	int n_prod_st = cur_nd->cfstate;
        ...
        if(cur_nd->coord.x==dest->coord.x && cur_nd->coord.y==dest->coord.y && cur_nd->cfstate==dest->cfstate)
        {
	  ...
	  vector<int> transition_req = cost_filter[n_prod_st][prod_states[pro]][lp];
	   ...
	   //finding the propositions true at neighbour cell nbh in grid//
	    ...
	    if(trans_literals_satisfied==transition_req.size())
	    {
	      ...
	      neigh_state.push_back(prod_states[pro]);
	      float l = trans_cost[prod_states[pro]];
	      float k = ((abs(nbh.y-cur_pt.y)+abs(nbh.x-cur_pt.x))>1?1.5:1);
              ...
              {   //cntstate++;
                ...
                trans_system_path_node* neigh_node = newtrans_system_path_node(nbh.x,nbh.y,current_automaton_state,prod_states[pro]);
		neigh_node->t_cst = l;//power(prefs,prefs+2);                
		neigh_node->g = cur_nd->g+k+l-cur_nd->t_cst;
		...
		neigh_node->f = neigh_node->g+neigh_node->h;
              ...
              else
              {
                ...
                if(oldtmp->g > (cur_nd->g+k+l-cur_nd->t_cst) )
                {
        ...
        if(nd->par!=NULL)
        {
            ...
            if(!updated[par_node][cur_node])
            {
                ...
                if(negtrans[par_automaton_state][par_automaton_state].size() > 0)
                {
                    ...
                    copy_map(obs1,-1);    
                    float tmp_dis = cal_actual_distance_bw_points(par_coord,cur_coord,nd->cfstate,nd->par->cfstate,nd->par->t_cst);
			//cout<<" tmp dis is "<<tmp_dis<<"\n";
                ...
                {
                    adj[par_node][cur_node] = cal_actual_distance_bw_points(par_coord,cur_coord,nd->cfstate,nd->par->cfstate,nd->par->t_cst);
                    updated[par_node][cur_node] = 1;
                }
                total_updates++;
            }
...
void expand_prod_graph_node(priority_queue<prod_graph_node*,vector<prod_graph_node*>,prod_graph_comparator> &qopen,prod_graph_node* cur_nd,unordered_map<long long int,prod_graph_node*> &init_vertex,int cfs)
{
    ...
    //cout<<cur_pt.y<<","<<cur_pt.x<<","<<cur_nd->state<<","<<cur_nd->cfstate<<"->"<<cur_nd->f<<"\n";
	      ...
	      //{	cfs=prod_states[pro];		
	        ...
	        else
	        {
	            ...
	            for(int z=0;z<qsytrans[current_automaton_state][j].size();z++)
	            {
                       ...
                       vector<int> neigh_node_info{pos_system_state[neigh_state][0],pos_system_state[neigh_state][1],j,cfs};
	...
	float cur_cost = neigh_set[i].t_cst;	
	cur_cost = neigh_set[i].t_cst-cur_nd->t_cst;

        //prod_graph_node* tmp = new_prod_graph_node(neigh_state,neigh_set[i].state,nbh.y,nbh.x);
	
	cost_val = cur_cost+cal_heuristic_cost(cur_pt,nbh);
	...
	//cout<<tval<<"\n";
	//cout<<neigh_set[i].coord.y<<" "<<neigh_set[i].coord.x<<" "<<neigh_set[i].state<<" "<<neigh_set[i].cfstate<<" "<<cur_cost<<"-->";
...
int find_path_in_prod_graph(prod_graph_node* srcnode,prod_graph_node* dest_node,int degen)
{
    ...
    for(iter=1; iter>0 ;iter++)
    {
	   ...
	   // cout<<"------------------------destination state"<<dest_node->cfstate<<"\n";
            expand_prod_graph_node(qopen,cur_nd,init_vertex,dest_node->cfstate);
            closed.push_back(cur_nd);
